...
<div class="bg-[#323231] rounded-lg p-8 shadow-lg max-w-sm w-full">
    ...
    <form id="addPaymentForm" class="text-white" method="POST" action="index.php?controller=payment&amp;action=edit">
        ...
        <div class="relative flex items-center mb-4">
            <select id="payerName" name="payerName"
                class="px-4 py-3 bg-[#323231] w-full text-sm border outline-[#edb705] rounded transition-all">
                <option selected value="<?= $payment->getPayerName() ?>" name="<?= $currentuser ?>">
                    <?= $payment->getPayerName() ?>
                </option>
                <?php foreach ($members as $member): ?>
                    <?php if ($member["userName"] == $payment->getPayerName())
                        continue; ?>
