        ...
        <ul class="flex justify-center space-x-10 text-white text-lg font-bold">
            ...
            <li>
                <a href="index.php?controller=proyectos&amp;action=viewMembers&amp;id=<?= $proyecto->getId() ?>"
                    class="hover:text-<?=$proyecto->getTheme()?>"><?=i18n(key: "Members") ?></a>
            </li>
        </ul>
    </nav>
</div>
...
<div class="bg-[#323231] rounded-lg p-8 shadow-lg max-w-4xl w-full text-center mt-2">
    ...
    <div
        ...
        <?php if ($gastos != null): ?>
            <?php foreach ($gastos as $payment): ?>
                ...
                <div class="flex flex-col justify-between bg-[#3d3d3d] p-4 rounded text-white space-y-2">
                    ...
                    <div class="flex justify-between items-center">
                        <div class="flex items-center space-x-2">
                            ...
                            <span><?= $payment->getDebt() ?></span>
                        </div>
                        <div class="flex items-center space-x-4">
                            <b>$<?= $payment->getTotalAmount() ?></b>
                            <a
                                href="index.php?controller=gastos&amp;action=edit&amp;id=<?= $proyecto->getId() ?>&amp;idPayment=<?= $payment->getPaymentId() ?>">
                    ...
                    <div class="relative flex justify-between items-center text-sm">
                        <p><?= $payment->getPayerId() ?></p>
                        <i class="fa-solid fa-arrow-right text-white absolute left-1/2 transform -translate-x-1/2"></i>
                        <div class="flex items-center space-x-4">
                            <button data-modal-target="modal-viewmemberspago<?= $payment->getPaymentId() ?>"
                                data-modal-toggle="modal-viewmemberspago<?= $payment->getPaymentId() ?>"
                                class="text-white hover:text-yellow-400" type="button">
                                <p class="underline"><?= count($payment->getDebtors()) ?> usuarios</p>
                            </button>
                            <button data-modal-target="modal-deletepago<?= $payment->getPaymentId() ?>"
                                data-modal-toggle="modal-deletepago<?= $payment->getPaymentId() ?>"
                ...
                <div id="modal-viewmemberspago<?= $payment->getPaymentId() ?>" tabindex="-1" aria-hidden="true"
                ...
                <div id="modal-deletepago<?= $payment->getPaymentId() ?>" tabindex="-1" aria-hidden="true"
                    class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative p-4 max-w-2xl max-h-full">
                        <!-- Modal content -->
                        <div class="relative bg-white rounded-lg shadow dark:bg-[#323231]">
                            ...
                            <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                                ...
                                <form id="deleteProject" class="text-white" method="POST"
                                    action="index.php?controller=gastos&amp;action=delete">
                                    <input type="text" value="<?= $payment->getPaymentId() ?>" name="id" hidden>
                                    <button type="submit"
    ...
    <div class="flex justify-center mt-4">
        <a href="index.php?controller=gastos&amp;action=add&amp;id=<?= $proyecto->getId() ?>">
