...
<div class="bg-[#323231] rounded-lg p-8 shadow-lg max-w-sm w-full">
    ...
    <form id="addPagoForm" class="text-white" method="POST" action="index.php?controller=gastos&amp;action=add">
        ...
        <div class="relative flex items-center mb-4">
            <select id="payerId" name="payerId"
                class="px-4 py-3 bg-[#323231] w-full text-sm border outline-[#edb705] rounded transition-all">
                <option selected value="<?= $payment->getPayerId() == "" ? $currentuser : $payment->getPayerId() ?>"
                    name="<?= $payment->getPayerId() == "" ? $currentuser : $payment->getPayerId() ?>">
                    <?= $payment->getPayerId() == "" ? $currentuser : $payment->getPayerId() ?>
                </option>
